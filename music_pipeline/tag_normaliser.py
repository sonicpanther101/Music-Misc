...
def apply_changes(folder_path, tag, standard_value, variations, store):
    ...
    if others and all(is_case_only(other, standard_value) for other in others):
        ...
        # Reuse an earlier answer for this exact standardisation, if any.
        key_old = "\n".join(sorted(others))
        remembered = store.get(f"normalise:{tag}", key_old, standard_value)
        if remembered is not None:
            verdict = "applying" if remembered else "skipping"
            print(f"  (remembered your earlier answer - {verdict})")
            confirm = 'y' if remembered else 'n'
        else:
            confirm = input("\nApply these changes? (y/n): ").strip().lower()
            store.set(f"normalise:{tag}", key_old, standard_value, confirm == 'y')
...
def normalise(path = None, store=None):
    if store is None:
        store = DecisionStore()
    ...
    for i, group in enumerate(inconsistency_groups, 1):
        ...
        if standard:
            apply_changes(folder_path, tag, standard, variations, store)
